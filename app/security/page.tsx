      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-96 border-r border-slate-800 bg-slate-900/50 p-4 flex flex-col space-y-4">
          <div>
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-sm font-semibold text-slate-200">Security Impact Analyzer</h2>
            </div>
          ...
          <div className="p-3 rounded-lg bg-slate-950 border border-slate-800 text-xs space-y-1.5">
            <span className="text-[11px] text-slate-500 block font-medium">Quick Demo Targets:</span>
            <div className="flex flex-wrap gap-1.5">
              {['lodash', 'express', 'axios', 'auth'].map((t) => (
                <button
                  ...
                  className="px-2 py-1 rounded bg-slate-800 text-slate-300 font-mono hover:bg-slate-700 text-[11px]"
                >
                  {t}
                </button>
              ))}
          ...
          {impactResult && (
            <div className="p-3.5 rounded-lg bg-slate-950 border border-slate-800 space-y-3 text-xs">
              <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                <span className="font-semibold text-slate-200">Impact Summary</span>
                <span className="px-2 py-0.5 rounded bg-amber-950/60 text-amber-400 border border-amber-800/80 font-mono text-[10px]">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {impactResult.metrics.affectedServices.map((s: string) => (
                      <span key={s} className="px-2 py-0.5 rounded bg-purple-950/50 border border-purple-800/60 text-purple-300 text-[10px] font-mono">
                        {s}
                      </span>
                    ))}
                  </div>
              ...
              <p className="text-[10px] text-slate-500 italic border-t border-slate-800 pt-2 leading-tight">
        ...
        <div className="flex-1 p-4 bg-slate-950">
          {impactResult ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-500 border border-slate-800 rounded-lg bg-slate-950">
              <ShieldAlert className="w-12 h-12 text-slate-600 mb-3" />
              <p className="text-sm font-medium text-slate-300">Enter a target package or service to analyze impact</p>
              <p className="text-xs text-slate-600 mt-1">Traces dependency exposure paths through CognoDB multi-hop graph queries.</p>
            </div>
          )}
        </div>
      </div>
