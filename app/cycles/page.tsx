  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-hidden transition-colors duration-200">
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-96 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-4 flex flex-col space-y-4">
          <div>
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Cycle Analyzer</h2>
            </div>
          ...
          {cycles.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center bg-slate-100 dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800">
              <CheckCircle2 className="w-8 h-8 text-emerald-500 mb-2" />
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">Zero Circular Dependencies</p>
              <p className="text-[11px] text-slate-500 dark:text-slate-500 mt-1">No circular dependency paths found in this repository.</p>
            </div>
          ) : (
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg border text-xs font-mono transition ${
                      isSelected
                        ? 'bg-red-50 dark:bg-red-950/40 border-red-500/80 ring-1 ring-red-500/50 text-slate-900 dark:text-white'
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="font-bold text-red-600 dark:text-red-400">{cycle.id}</span>
                      <span className="px-2 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-[10px] text-slate-600 dark:text-slate-400">
                        {cycle.length} Nodes
                      </span>
                    </div>
                    <p className="text-slate-800 dark:text-slate-300 break-all leading-relaxed">{cycle.pathString}</p>
                  </button>
                );
              })}
            </div>
        ...
        <div className="flex-1 p-4 bg-slate-100 dark:bg-slate-950 flex flex-col">
          {selectedCycle && (
            <div className="mb-3 px-4 py-2.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs font-mono shadow-sm">
              <span className="text-slate-600 dark:text-slate-400">
                Focused Cycle Loop: <strong className="text-red-600 dark:text-red-400">{selectedCycle.id}</strong>
              </span>
              <span className="text-slate-800 dark:text-slate-300">{selectedCycle.pathString}</span>
            </div>
          )}
